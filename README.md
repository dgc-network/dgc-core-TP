...
The customer is identified by a customer name and a corresponding public key. The wallet account balance, is stored at an address, derived from SHA 512 hash of customer's public key and the dgc-core transaction family namespace.
...
Start the pre-built Docker containers in docker-compose.yaml file, located in dgc-core-TP directory:
```bash
cd dgc-core-tp
docker-compose up
```
...
```bash
docker exec -it dgc-core-client bash
```
...
dgc-core deposit 100 jack #This adds 100 units to Jack's account

dgc-core withdraw 50 jack #Withdraws 50 units from Jack's account

dgc-core balance jack #Displays the balance left in Jack's account

#Create 2nd wallet
sawtooth keygen jill #This creates the public/private keys for Jill, a pre-requisite for the following commands

dgc-core deposit 100 jill #This adds 100 units to Jill's account

dgc-core balance jill #Displays the balance left in Jill's account

dgc-core transfer 25 jack jill #Transfer 25 units money from Jack to Jill
...
# Building containers
To build TP code of your preferred language and run the dgc-core example:

```bash
docker-compose -f dgc-core-build-tp-<your_prog_language>.yaml up --build
```
where,
 <your_prog_language> should be replaced with either `cxx`, `java`, or `py`

# Building and running on OS(without dockers)
To run dgc-core-TP without dockers, we'll have to use a Ubuntu 16.04 OS installation and compile dgc-core from sources. Below is a sample procedure for python TP/client:
...
1. Install sawtooth on Ubuntu 16.04 LTS x64 machine and setup genesis block. Refer sawtooth app developer's guide [here](https://sawtooth.hyperledger.org/docs/core/releases/latest/app_developers_guide/ubuntu.html)
   - Start the validator, rest-api and settings-tp in separate, new consoles:
     ...
     ```
2. In a new console, clone the dgc-core repo:

   `git clone https://github.com/dgc-network/dgc-core-tp.git`
3. `cd dgc-core-tp`
4. Modify two files:
...
5. Setup the dgc-core-tp.
   - Follow the [`pyprocessor/Dockerfile`](https://github.com/dgc-network/dgc-core-tp/blob/master/pyprocessor/Dockerfile)
   - Install all the dependencies in the [first `RUN` line](https://github.com/dgc-network/dgc-core-tp/blob/master/pyprocessor/Dockerfile#L18) in the Dockerfile
   - Run the dgc-core-tp with `./pyprocessor/dgc-core-tp`
6. Setup the client. Open a new console.
   - Follow the [`pyclient/Dockerfile`](https://github.com/dgc-network/dgc-core-tp/blob/master/pyclient/Dockerfile)
   - Install all the dependencies in the [first `RUN` line](https://github.com/dgc-network/dgc-core-tp/blob/master/pyclient/Dockerfile#L20) in the pyclient/Dockerfile
   - Run the dgc-core client with `./pyclient/dgc-core` command. Refer [Usage](#Usage) section above, for examples.

**NOTE** If you prefer using the dgc-core client without directory prefix, you could add the `pyclient` directory to your `PATH` environment variable, as shown below:

`export PATH = $PATH:<absolute-path-to-your-cloned-dgc-core-TP-dir>/pyclient"`
...
More ways you can contribute:
 - Bugs or issues: Report problems or defects found when working with dgc-core
